...
def comparing_positions(tracks1, tracks2):
    """
    Create a distance matrix between two sets of tracks
    Return the matched indices and the unmatched indices
    """
    M = len(tracks1)
    N = len(tracks2)
    ...
    for i, track1 in enumerate(tracks1):
        for j, track2 in enumerate(tracks2):
            distance_matrix[i, j] = eucl2D(track1['translation'], track2['translation'])
            if track1['tracking_name'] != track2['tracking_name']:
                distance_matrix[i, j] = 1e18

    matched_indices = greedy_assignment(deepcopy(distance_matrix))
    unmatched_tracks1_data = [d for d in range(len(tracks1)) if not (d in matched_indices[:, 1])]
    unmatched_tracks2_data = [d for d in range(len(tracks2)) if not (d in matched_indices[:, 0])]
    
    return matched_indices, unmatched_tracks1_data, unmatched_tracks2_data
